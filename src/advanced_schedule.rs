...
pub struct AdvancedScheduleWidget {
    should_exit: bool,
    selected_type: AdvancedSchedule,
    list_element_entries: AdvancedScheduleList
}

struct AdvancedScheduleList {
    list: Vec<AdvancedScheduleEntry>,
    state: ListState
}

impl FromIterator<(AdvancedSchedule, &'static str)> for AdvancedScheduleList {
    fn from_iter<I: IntoIterator<Item = (AdvancedSchedule, &'static str)>>(iter: I) -> Self {
        let list = iter
            .into_iter()
            .map(|(list_element, info)| AdvancedScheduleEntry::new(list_element, info))
            .collect();
        let state = ListState::default();
        Self { list, state }
    }
}
struct AdvancedScheduleEntry {
    list_element: AdvancedSchedule,
    info: String,
}
...
impl AdvancedScheduleEntry {
    fn new(list_element: AdvancedSchedule, info: &str) -> Self {
        Self {
            list_element,
            info: info.to_string()
        }
...
impl Default for AdvancedScheduleWidget {
    fn default() -> Self {
        Self {
            should_exit: false,
...
impl AdvancedScheduleWidget {
    ...
    fn set_current_type(&mut self) {
        if let Some(i) = self.list_element_entries.state.selected() {
            match self.list_element_entries.list[i].list_element {
                AdvancedSchedule::Yes => self.selected_type = AdvancedSchedule::Yes,
                AdvancedSchedule::No => self.selected_type = AdvancedSchedule::No,
            }
        }
    }
    ...
    fn render_list(&mut self, area: Rect, buf: &mut Buffer) {
        let block = Block::new()
            .title(Line::raw("Do you want to set up an advanced schedule?").centered())
            .borders(Borders::TOP)
...
impl Widget for &mut AdvancedScheduleWidget {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        AdvancedScheduleWidget::render_header(header_area, buf);
        AdvancedScheduleWidget::render_footer(footer_area, buf);
